              ...
              <div key={index} className="mb-16 ml-14 sm:ml-20 transform transition-all duration-300 hover:scale-[1.02]">
                ...
                <div className="bg-white/60 dark:bg-gray-800/50 backdrop-blur-lg p-4 pl-6 sm:p-5 lg:p-6 rounded-xl shadow-lg border border-gray-200/50 dark:border-gray-800/50 hover:border-indigo-500/30 transition-colors max-w-full sm:max-w-2xl lg:max-w-3xl">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-2 mb-2">
                    <h3 className="text-base sm:text-lg font-bold text-indigo-600 dark:text-indigo-400 flex-1 leading-tight">{item.degree}</h3>
                    <span className="bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300 text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap self-start">{item.duration}</span>
                  </div>
                  <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-block">
                    <h4 className="text-sm sm:text-base font-semibold text-gray-700 dark:text-gray-300 mb-2 hover:underline hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">{item.institution}</h4>
                  </a>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
